...
Rake::Task['release'].enhance do
  ...
  system 'open https://github.com/g2crowd/gem/releases'
end

desc 'verify that all files in gemspec.files are tracked by git'
...
namespace :bump do
  desc 'Update the version of Bundler in Gemfile.lock to the latest version'
  task :bundler do
    version = Gem.latest_version_for('bundler').to_s
    replace_in_file 'Gemfile.lock', /^BUNDLED WITH\n\s+(\d\S+)$/ => version
  end

  desc 'Update the version of Ruby in example.gemspec, .rubocop.yml, and .github/workflows/ci.yml'
  task :ruby do
  ...
  desc 'Update the year in LICENSE.txt'
  task :year do
